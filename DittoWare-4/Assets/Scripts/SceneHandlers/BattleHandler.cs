...
public class BattleHandler : MonoBehaviour
{
    ...
    {
        float distanceX = pokemonStatsDisplay[position].rectTransform.sizeDelta.x + 128f;
        float startX = (retract) ? 157f + 64f - (distanceX / 2) : 157f + 64f + (distanceX / 2);
        ...
        for (int i = 0; i < 4; i++)
        {
            if (moveset[i] != null)
            {
                ...
                if (type == PokemonData.Type.BUG)
                {
                    ...
                    buttonMoveCover[i].sprite = Resources.Load<Sprite>("PCSprites/moveButton" + type.ToString());
            ...
            {
                buttonMoveCover[i].sprite = Resources.Load<Sprite>("null");
    ...
    private void updateSelectedMove(int newPosition)
    {
        ...
        buttonMoveCover[0].color = (movePosition == 1) ? new Color(0.55f, 0.55f, 0.55f, 1) : new Color(0.5f, 0.5f, 0.5f, 1);
        buttonMoveCover[1].color = (movePosition == 2) ? new Color(0.55f, 0.55f, 0.55f, 1) : new Color(0.5f, 0.5f, 0.5f, 1);
        buttonMoveReturn.sprite = (movePosition == 3) ? buttonReturnSelTex : buttonReturnTex;
        buttonMoveCover[2].color = (movePosition == 4) ? new Color(0.55f, 0.55f, 0.55f, 1) : new Color(0.5f, 0.5f, 0.5f, 1);
        buttonMoveCover[3].color = (movePosition == 5) ? new Color(0.55f, 0.55f, 0.55f, 1) : new Color(0.5f, 0.5f, 0.5f, 1);
    }
    ...
    private IEnumerator drawTextAndWait(string message, float time, float lockedTime, bool silent)
    {
        ...
        if (time > 0)
        {
            while (!Input.GetButtonDown("Select") && !Input.GetButtonDown("Back") && Time.time < startTime + time && !CrossPlatformInputManager.GetButtonDown("A") && !CrossPlatformInputManager.GetButtonDown("B"))
            {
                yield return null;
            }
        }
    ...
    public IEnumerator control(bool isTrainerBattle, Trainer trainer, bool healedOnDefeat)
    {
        ...
        playerBase.sprite = PlayerMovement.player.accessedMapSettings.getBattleBase(currentTileTag, true);
        opponentBase.sprite = PlayerMovement.player.accessedMapSettings.getBattleBase(currentTileTag, false);
...
//		Debug.Log(pokemon[3].getName()+": HP: "+pokemon[3].getHP()+"ATK: "+pokemon[3].getATK()+"DEF: "+pokemon[3].getDEF()+"SPA: "+pokemon[3].getSPA()+"SPD: "+pokemon[3].getSPD()+"SPE:"+pokemon[3].getSPE());
        ...
        if (trainerBattle)
        {
            ...
            StartCoroutine(slidePokemon(opponentBase, opponent1, false, false, new Vector3(86, 124, 0)));
            StartCoroutine(slidePokemon(playerBase, player1, false, true, new Vector3(-80, 68, 0)));
        ...
        else
        {
            ...
            StartCoroutine(slidePokemon(opponentBase, opponent1, true, false, new Vector3(86, 124, 0)));
            yield return StartCoroutine(slidePokemon(playerBase, player1, false, true, new Vector3(-80, 68, 0)));
            Dialog.DrawDialogBox();
        ...
        while (running)
        {
            ...
            runState = true;
            Dialog.DrawDialogBox();
            StartCoroutine(Dialog.DrawTextSilent("What will " + pokemon[0].getName() + " do?"));
            while (runState)
            {
