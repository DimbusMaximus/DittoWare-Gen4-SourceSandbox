...
public class BagHandler : MonoBehaviour
{
    ...
    public GUITexture DescBar;
    public GUITexture DescBarIcon;
    ...
    private void updateSelectedItem()
    {
        for (int i = 0; i < 8; i++)
        {
            if (currentPosition[currentScreen] == i)
            {
                ...
                DescBarIcon.texture = itemIcon[i].texture;
            }
            else
            {
                if (switching)
    ...
    private void updateDescription()
    {
        ...
        if (index < currentItemList.Length)
        {
            ...
            if (currentScreen != 4)
            {
                ...
                DescBar.enabled = true;
                DescBarIcon.enabled = true;
            }
            else if (!shopMode)
            {
                ...
                DescBar.enabled = false;
                DescBarIcon.enabled = false;
            }
        }
        else
        {
    ...
    private IEnumerator animateSelection(int selectionPosition, int deselectionPosition)
    {
        ...
        updateSelectedItem();
        DescBarIcon.texture = itemIcon[selectionPosition].texture;
    }
    ...
    private IEnumerator control(bool partyAccessible, bool getItem, bool setShopMode, string[] shopStock)
    {
        ...
        while (running)
        {
            ...
            else if (Input.GetButton("Select") || CrossPlatformInputManager.GetButton("A"))
            {
                                ...
                                else
                                {
                                    ...
                                    else if (partyLength > 0)
                                    {
                                        ...
                                        while (selectingPokemon)
                                        {
                                            ...
                                            else if (Input.GetAxisRaw("Horizontal") > 0 || Input.GetButton("Back") && !CrossPlatformInputManager.GetButtonDown("A") && !CrossPlatformInputManager.GetButtonDown("B"))
                                            {
                                                selectingPokemon = false;
                                                switching = false;
                                                selected = -1;
