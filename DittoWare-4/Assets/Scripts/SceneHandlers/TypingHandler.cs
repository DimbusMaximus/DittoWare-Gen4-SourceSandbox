...
public class TypingHandler : MonoBehaviour
{
    ...
    private IEnumerator swapPages(int newPageIndex)
    {
        if (newPageIndex >= 0 && newPageIndex < 4 && newPageIndex != pageIndex)
        {
            ...
            while (increment < 1f)
            {
                ...
                page[newPageIndex].localPosition = new Vector3(-300f * (1 - increment), -128, 0);
                page[pageIndex].localPosition = new Vector3(0, -128 - 140f * increment, 0);
        ...
        Sprite[] iconAnimation)
    {
        ...
        page[0].localPosition = new Vector3(0, -128, 0);
        for (int i = 1; i < 4; i++)
        {
            page[i].localPosition = new Vector3(-300f, -128, 0);
        }
        ...
        while (running)
        {
            ...
            if (Input.GetButtonDown("Start") || CrossPlatformInputManager.GetButtonDown("Y"))
            {
                setSelectorPosition(5);
                yield return new WaitForSeconds(buttonDelay);
            }
