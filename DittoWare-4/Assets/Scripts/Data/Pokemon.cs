...
public class Pokemon
{
    ...
    private static Sprite[] GetAnimFromID_(string folder, int ID, Gender gender, bool isShiny, int form = 0)
    {
        //Alolan Shiny Female Bulbasaur: PokemonSprites/001fs_1.png
        Sprite[] animation;
        string shiny = (isShiny) ? "s" : "";
        string file = folder + "/" + convertLongID(ID);
        string form_number = "_" + form;

        //Attempt to load Female Variant
        if (gender == Gender.FEMALE)
        {
            file += "f";
            animation = Resources.LoadAll<Sprite>(file);
        ...
        if (form > 0)
        {
            ...
            if (animation.Length == 0)
            {
                Debug.LogWarning("Form Variant NOT Found");
                file = file.Remove(file.Length - 1, form_number.Length);
            }
        }
        //Attempt to load Base Sprite
        animation = Resources.LoadAll<Sprite>(file);
        if (animation.Length == 0)
        {
            Debug.LogWarning("Base Sprite NOT Found");
        }
    ...
    private static Texture[] GetAnimFromID(string folder, int ID, Gender gender, bool isShiny, int form = 0)
    {
        //Alolan Shiny Female Bulbasaur: PokemonSprites/001fs_1.png
        Texture[] animation;
        string shiny = (isShiny) ? "s" : "";
        ...
        {
            file += "f";
            animation = Resources.LoadAll<Texture>(file);
            if (animation.Length == 0)
            {
                Debug.LogWarning("Female Variant NOT Found");
                file = file.TrimEnd(file[file.Length - 1]);
