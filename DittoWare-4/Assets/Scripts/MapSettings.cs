...
public class MapSettings : MonoBehaviour
{
    ...
    public Sprite getBattleBase(bool player = false)
    {
        return getBattleBase(0, player);
    }

    public Sprite getBattleBase(int currentTag, bool player = false)
    {
        float time = System.DateTime.Now.Hour + ((float) System.DateTime.Now.Minute / 60f);
        string timeString = "";
        string side = (player) ? "_player" : "";
        if (time >= 20 || time < 3.5f)
        {
            //night
            timeString = "Night";
        ...
        {
            //return with timestring attached
            return Resources.Load<Sprite>("BattleBackgrounds/Bases/" + checkString + timeString + side);
        }

        //else return without timestring
        return Resources.Load<Sprite>("BattleBackgrounds/Bases/" + checkString + side);
    }
