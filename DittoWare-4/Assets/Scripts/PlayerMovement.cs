...
public class PlayerMovement : MonoBehaviour
{
    ...
    //private ScreenOrientation Screen2;
    ...
    public bool runningforced = false;
    public bool surfing = false;
    public bool bike = false;
    public bool strength = false;
    public float walkSpeed = 0.266f; //time in seconds taken to walk 1 square.
    public float runSpeed = 0.133f;
...
//	private SceneTransition sceneTransition;
    ...
    public BottomScreen BottomOWMenu;
    ...
    void Awake()
    {
        ...
        int resWidth = Screen.currentResolution.width; //add a buffer  orig value: + 10
        int resHeight = Screen.currentResolution.height; //add a buffer  orig value: + 6
        ...
        walkClip = Resources.Load<AudioClip>("Audio/sfx/step_norm");

        BottomOWMenu = GameObject.Find("Global/BottomScreen/Menu").GetComponent<BottomScreen>();
    }
    ...
    void Update()
    {
        //Debug Character Outfit Change
        if (Input.GetKeyDown("1")){
            SaveData.currentSave.isMale = true;
            SaveData.currentSave.playerOutfit = "hgss";
        }
        if (Input.GetKeyDown("2")){
            SaveData.currentSave.isMale = false;
            SaveData.currentSave.playerOutfit = "hgss";
        }
        if (Input.GetKeyDown("3")){
            SaveData.currentSave.isMale = true;
            SaveData.currentSave.playerOutfit = "plat";
        }
        ...
        if (Input.GetKey("["))
        {
            if (mainCamera.transform.eulerAngles.y >= 0 && mainCamera.transform.eulerAngles.y < 90)
            {
                mainCamera.transform.position += new Vector3(-0.25f, 0, 0.25f);
            }
            if (mainCamera.transform.eulerAngles.y >= 90 && mainCamera.transform.eulerAngles.y < 180)
            {
                mainCamera.transform.position += new Vector3(0.25f, 0, 0.25f);
            }
            if (mainCamera.transform.eulerAngles.y >= 180 && mainCamera.transform.eulerAngles.y < 270)
            {
                mainCamera.transform.position += new Vector3(0.25f, 0, -0.25f);
            }
            if (mainCamera.transform.eulerAngles.y >= 270 && mainCamera.transform.eulerAngles.y < 360)
            {
                mainCamera.transform.position += new Vector3(-0.25f, 0, -0.25f);
            }
            if (mainCamera.transform.eulerAngles.y >= 360)
            {
                mainCamera.transform.localRotation = Quaternion.Euler(45f,0,0) ;
            }
            mainCamera.transform.eulerAngles += new Vector3(0, 0.9f, 0);
        }
        ...
        if (Input.GetKey("]"))
        {
            if (mainCamera.transform.eulerAngles.y >= 0 && mainCamera.transform.eulerAngles.y < 90)
            {
                mainCamera.transform.eulerAngles -= new Vector3(0, 90f / 100, 0);
                mainCamera.transform.position -= new Vector3(-24.55f / 100, 0, 24.55f / 100);
            }
        }
        ...
        else if (Input.GetButtonDown("Vertical") || CrossPlatformInputManager.GetButtonDown("Vertical"))
        {
    ...
    private bool isDirectionKeyHeld(int directionCheck)
    {
        ...
        if (directionCheck == 0 && Input.GetAxisRaw("Vertical") > 0 || CrossPlatformInputManager.GetAxisRaw("Vertical") > 0)
        {
            directionHeld = true;
        }
        else if (directionCheck == 1 && Input.GetAxisRaw("Horizontal") > 0 || CrossPlatformInputManager.GetAxisRaw("Horizontal") > 0)
        {
            directionHeld = true;
        }
        else if (directionCheck == 2 && Input.GetAxisRaw("Vertical") < 0 || CrossPlatformInputManager.GetAxisRaw("Vertical") < 0)
        {
            directionHeld = true;
        }
        else if (directionCheck == 3 && Input.GetAxisRaw("Horizontal") < 0 || CrossPlatformInputManager.GetAxisRaw("Horizontal") < 0)
        {
            directionHeld = true;
        }
    ...
    private IEnumerator control()
    {
        ...
        while (true)
        {
            ...
            if (canInput)
            {
                if (!surfing && !bike)
                {
                    if (Input.GetButton("Run") || CrossPlatformInputManager.GetButton("B") || runningforced)
                    {
                        running = true;
                        if (moving)
                        {
                ...
                if (Input.GetButton("X") || CrossPlatformInputManager.GetButton("X"))
                {
                    //open Pause Menu
                    if (moving || Input.GetButtonDown("X") || CrossPlatformInputManager.GetButtonDown("X"))
                    {
                ...
                else if (Input.GetButtonDown("Select") || CrossPlatformInputManager.GetButtonDown("A"))
                {
                    interact();
                }
                //if pausing/interacting/etc. is not being called, then moving is possible.
                //		(if any direction input is being entered)
                else if (Input.GetAxisRaw("Horizontal") != 0 || Input.GetAxisRaw("Vertical") != 0 || CrossPlatformInputManager.GetAxisRaw("Horizontal") != 0 || CrossPlatformInputManager.GetAxisRaw("Vertical") != 0)
                {
                ...
                else if (Input.GetButtonDown("Y") || CrossPlatformInputManager.GetButtonDown("Y"))
                {
                    //DEBUG
    ...
    {
        BottomOWMenu.menuEnabled = false;//disables menu
        canInput = false;
    ...
    {
        BottomOWMenu.menuEnabled = true;//enables menu
        Debug.Log("unpaused");
        canInput = true;
    }
    ...
    private void playClip(AudioClip clip, float vol = 1f)
    {
        PlayerAudio.clip = clip;
        PlayerAudio.volume = PlayerPrefs.GetFloat("sfxVolume") * vol;
        PlayerAudio.Play();
    }
